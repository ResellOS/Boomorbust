...
export async function fetchRotationData(
  ...
  // Market verdicts (BUY/SELL vs KTC) — computed MARKET-WIDE across the full
  // scored skill pool so a player's verdict is league-relative, not roster-
  // relative. Skill-scoped (QB/RB/WR/TE in formula_scores); KTC from bbv_values.
  const marketVerdictByPlayer = new Map<string, PlayerMarketVerdict>();
  try {
    ...
    if (poolIds.length > 0) {
      // Positions for the full pool (reuse rostered meta, fetch the rest).
      const posById = new Map<string, string>();
      const needPos = poolIds.filter((id) => !metaByPlayer.has(id));
      for (const id of poolIds) {
        const m = metaByPlayer.get(id);
        if (m) posById.set(id, m.position);
      }
      for (let i = 0; i < needPos.length; i += 200) {
        const { data } = await supabase
          .from('players')
          .select('id, position')
          .in('id', needPos.slice(i, i + 200));
        for (const p of data ?? []) posById.set(String(p.id), (p.position ?? '—').toUpperCase());
      }

      // KTC market values for the pool.
    ...
    console.error('[dashboard] market verdict compute failed:', err);
  }
