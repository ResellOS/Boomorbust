...
export interface LineupRecommendation {
  ...
  /** When available (engine internal), matchup defense rank vs position (1=h hardest) */
  matchup_rank?: number;
}
...
export async function generateLineupRecommendations(
  ...
  const starters = roster.filter((p) =>
    ['QB', 'RB', 'WR', 'TE', 'K', 'DEF'].includes(p.position?.toUpperCase() ?? '')
  );
  ...
  const recs = await Promise.all(
    starters.map(async (player): Promise<LineupRecommendation> => {
      ...
      const thresholds: Record<string, [number, number]> = {
        QB: [18, 14], RB: [12, 8], WR: [10, 7], TE: [8, 5], K: [8, 6], DEF: [9, 4],
      };
