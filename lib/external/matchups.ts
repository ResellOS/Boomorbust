...
/** Fantasy matchup grade 0–100 from opponent defensive rank (1 = toughest defense vs that phase). */
export function defenseRankToMatchupGrade(rank: number | undefined): number {
  const r = typeof rank === 'number' && rank >= 1 && rank <= 32 ? rank : 16;
  return Math.round(30 + ((32 - r) / 31) * 65);
}
