...
/** FantasyCalc public dynasty API — third-line fallback */
async function fetchFantasyCalc(): Promise<KTCPlayer[] | null> {
  try {
    ...
    const data = (await res.json()) as Array<{
      player: { name: string; position: string; age?: number };
      value: number;
      overallRank: number;
    }>;
    ...
    return data.map((entry) => ({
      ...
      slug: '',
      position: entry.player.position ?? 'UNK',
      age: entry.player.age ?? 0,
      ktc_value: entry.value ?? 0,
      rank: entry.overallRank ?? 0,
    })).filter((p) => p.player_name);
  } catch (err) {
    console.error('FantasyCalc fallback failed:', err);
    return null;
  }
}
...
export async function getKTCValues(): Promise<KTCPlayer[]> {
  ...
  const players =
    (await scrapeKTC()) ??
    (await fetchDynastyProcess()) ??
    (await fetchFantasyCalc()) ??
    [];
  ...
  if (players.length) {
    if (redis) {
      try {
        ...
        await redis.set('metrics:pipeline:ktc', new Date().toISOString(), { ex: TTL });
      } catch {}
    }
  } else {
    console.error('[ktc] all three sources failed — returning empty array');
  }
