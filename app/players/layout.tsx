...
export default async function PlayersLayout({
  ...
  try {
    ...
    if (error) console.error('[players/layout] getUser error:', error);
    ...
    if (userId) {
      ...
      if (profileError) console.error('[players/layout] profile error:', profileError);
      else if (!profile) console.error('[players/layout] no profile');
      else if (!profile.sleeper_user_id) needsOnboarding = true;
      else hasSleeper = true;
    }
  } catch (err) {
    console.error('[players/layout] auth failed:', err);
  }
