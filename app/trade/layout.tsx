...
export default async function TradeLayout({
  ...
  try {
    ...
    if (error) console.error('[trade/layout] getUser error:', error);
    ...
    if (userId) {
      ...
      if (profileError) console.error('[trade/layout] profile error:', profileError);
      else if (!profile) console.error('[trade/layout] no profile');
      else if (!profile.sleeper_user_id) needsOnboarding = true;
      else hasSleeper = true;
    }
  } catch (err) {
    console.error('[trade/layout] auth failed:', err);
  }
