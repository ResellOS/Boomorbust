...
async function runClearRedis(supabase: ReturnType<typeof createClient>): Promise<Response> {
  const {
    data: { user },
  ...
  try {
    ...
    const keys = ['ktc:dynasty:values', 'ktc_values', 'sleeper_players'];
    await Promise.all(keys.map((k) => redis.del(k)));
    return NextResponse.json({ message: `✓ Cleared ${keys.length} cache keys (${keys.join(', ')})` });
  } catch (e) {
