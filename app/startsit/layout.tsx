...
export default async function StartSitLayout({
  ...
  try {
    ...
    if (error) console.error('[startsit/layout] getUser error:', error);
    ...
    if (userId) {
      ...
      if (profileError) console.error('[startsit/layout] profile error:', profileError);
      else if (!profile) console.error('[startsit/layout] no profile');
      else if (!profile.sleeper_user_id) needsOnboarding = true;
      else hasSleeper = true;
    }
  } catch (err) {
    console.error('[startsit/layout] auth failed:', err);
  }
