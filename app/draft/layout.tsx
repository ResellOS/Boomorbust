...
export default async function DraftLayout({
  ...
  try {
    ...
    if (error) console.error('[draft/layout] getUser error:', error);
    ...
    if (userId) {
      ...
      if (profileError) console.error('[draft/layout] profile error:', profileError);
      else if (!profile) console.error('[draft/layout] no profile');
      else if (!profile.sleeper_user_id) needsOnboarding = true;
      else hasSleeper = true;
    }
  } catch (err) {
    console.error('[draft/layout] auth failed:', err);
  }
