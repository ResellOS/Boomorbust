  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        <div>
          <div className="font-mono text-[12px] uppercase text-text">{card.playerName}</div>
      ...
      <ul className="mt-3 space-y-1">
        {card.whyBullets.slice(0, 6).map((b) => (
          <li key={b} className="flex gap-1.5 font-mono text-[10px] leading-relaxed text-text">
            <span className="shrink-0 text-boom">✓</span>
            <span>{b}</span>
          </li>
        ))}
      </ul>
      <div className="mt-2 flex flex-wrap gap-3 font-mono text-[9px] text-muted">
