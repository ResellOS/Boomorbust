...
function TeamBlock({
  ...
  return (
    <div className="flex-1 text-center">
      <div className="mx-auto mb-1 flex h-10 w-10 items-center justify-center rounded-full border border-border bg-surface2 font-mono text-[11px] text-muted">
        ⬡
      </div>
      <div className="font-mono text-[11px] uppercase text-text">{label}</div>
      <div className="font-mono text-[18px] text-boom">
        <CountUpNumber value={score} resetKey={`${label}-${score}`} decimals={1} suffix=" pts" />
      </div>
      {syncing ? (
        <div className="font-mono text-[10px] text-hold">Opponent projection syncing</div>
      ) : winPct != null ? (
    ...
    return (
      ...
      >
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 font-mono text-[11px]">
          <div>
            <div className="text-[9px] uppercase text-muted">Total Projected</div>
            <div className="text-[17px] text-boom">
              <CountUpNumber
          ...
          <div>
            <div className="text-[9px] uppercase text-muted">Active Leagues</div>
            <div className="text-[17px] text-text">{portfolio.leagueCount}</div>
          </div>
          {portfolio.biggestEdge && (
            <div className="col-span-2">
              <div className="text-[9px] uppercase text-muted">Biggest Edge</div>
              <div className="text-[12px] text-boom">{portfolio.biggestEdge}</div>
            </div>
          )}
          {portfolio.closestMatchup && (
            <div>
              <div className="text-[9px] uppercase text-muted">Closest Matchup</div>
              <div className="text-[12px] text-hold">{portfolio.closestMatchup}</div>
            </div>
          )}
          {portfolio.highestRiskMatchup && (
            <div>
              <div className="text-[9px] uppercase text-muted">Highest Risk</div>
              <div className="text-[12px] text-bust">{portfolio.highestRiskMatchup}</div>
            </div>
          )}
        </div>
        <Link
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        />
        <div className="font-mono text-[12px] uppercase text-muted">vs</div>
        <TeamBlock
      ...
      {!matchup.syncing && matchup.projectedMargin != null && (
        <div className="mt-2 flex justify-between border-t border-border/60 pt-2 font-mono text-[11px]">
      ...
      {matchup.positionBreakdown.some((p) => p.you > 0) && (
        <div className="mt-2 border-t border-border/60 pt-2">
          <div className="mb-1 font-mono text-[9px] uppercase text-muted">Position Breakdown</div>
          <div className="grid grid-cols-4 gap-1">
            {matchup.positionBreakdown.map((p) => (
              <div key={p.slot} className="rounded bg-surface2/60 px-1 py-0.5 text-center">
                <div className="font-mono text-[8px] text-muted">{p.slot}</div>
                <div className="font-mono text-[10px] text-boom">{p.you.toFixed(1)}</div>
