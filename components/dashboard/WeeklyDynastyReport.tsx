...
export default function WeeklyDynastyReport({
  ...
  return (
    ...
    >
      ...
      <p className="text-[8px] font-mono uppercase tracking-[0.15em] text-[#64748B] px-0.5">
        Pre-generated summary · roster pulse (max 3)
      </p>
      {lines.length === 0 ? (
        <p className="text-[9px] text-[#475569] font-mono px-0.5 py-2">Sync completes to generate bullets.</p>
      ) : (
        <ul className="list-disc pl-4 space-y-1.5 pr-1">
          {lines.map((b, i) => (
            <li key={i} className="text-[9px] text-[#E2E8F0] leading-snug font-mono marker:text-[#FBBF24]">
              {b}
            </li>
          ))}
        </ul>
