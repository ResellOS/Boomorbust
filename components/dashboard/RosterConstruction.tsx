...
export default function RosterConstruction({
    ...
    return (
      ...
      >
        <div className="border-b border-[#1e2640]/80 px-3 py-2">
          ...
          <p className="font-mono text-[9px] text-[#8b9bb8]">Strength by room · tap for analysis</p>
        </div>
        ...
        <div className="border-t border-[#1e2640]/60 px-3 py-1.5">
          <span className="inline-flex items-center gap-0.5 font-mono text-[9px] text-boom">
  ...
  return (
    <Link href={href} className="dash-clickable-card block rounded-[10px] border border-[#1e2640] bg-[#0f1420] px-4 py-3 no-underline">
      ...
      <p className="mt-0.5 font-mono text-[9px] text-[#8b9bb8]">Where this roster is strong or weak</p>
...
function GradeTile({ g, dense }: { g: RosterConstructionGrade; dense?: boolean }) {
  return (
    <div className="rounded-md border border-[#1e2640]/50 bg-[#141929]/50 px-2 py-1.5">
      <div className="font-mono text-[8px] uppercase tracking-wide text-[#6b7a99]">{g.label}</div>
      <div className="mt-0.5 flex items-baseline gap-1">
        <span
          className={`font-mono font-semibold tabular-nums leading-none ${dense ? 'text-[15px]' : 'text-[18px]'}`}
          style={{ color: g.color }}
        >
          {g.letter}
        </span>
        <span className="truncate font-figtree text-[10px] text-[#8b9bb8]">{g.descriptor}</span>
      </div>
      {g.barFill > 0 ? (
