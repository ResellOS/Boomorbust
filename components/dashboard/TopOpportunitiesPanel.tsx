    ...
    <div className="flex flex-col overflow-hidden rounded-lg border border-border bg-surface">
      ...
      <div className="px-[13px] py-2.5">
        {opportunities.length === 0 ? (
          <p className="font-figtree text-[10px] text-muted">Opportunities sync with daily tasks.</p>
        ) : (
          opportunities.map((row) => (
            ...
            >
              <span className="min-w-0 truncate font-figtree text-[11px] text-text">{row.label}</span>
              <span className="shrink-0 font-mono text-[10px] tabular-nums text-boom">{row.impact}</span>
            </Link>
          ))
        )}
      </div>
