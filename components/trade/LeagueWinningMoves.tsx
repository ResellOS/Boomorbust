...
): { kind: MoveKind; candidates: TradeOpportunity[] }[] {
  const kinds: MoveKind[] = ['Best Buy Low', 'Best Sell High', 'Most Likely Accepted', 'Highest Impact'];
...
export default function LeagueWinningMoves({
  ...
  return (
    <section onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      ...
      <div
        className={`dash-priority-fade grid grid-cols-1 gap-2 transition-opacity duration-200 sm:grid-cols-2 ${visible ? 'opacity-100' : 'opacity-0'}`}
        style={{ gridTemplateColumns: `repeat(${Math.min(cards.length, 4)}, minmax(0, 1fr))` }}
      >
        {cards.map(({ kind, opp: o }, index) => {
          const tone = kind === 'Best Sell High' ? 'bust' : 'boom';
          const reason = o.reasonChips[0] ?? o.whyReasons[0] ?? o.portfolioImpactNote;
          return (
            <AnimatedCard key={`${kind}-${o.id}`} delay={index * 80} className="h-full">
              <GlowBorder tone={tone} rounded="rounded-[10px]" className="h-full">
                <button
                  type="button"
                  onClick={() => onSelect(o)}
                  className="dash-clickable-card h-full w-full rounded-[10px] border border-transparent bg-[#0f1420] p-2.5 text-left"
                >
