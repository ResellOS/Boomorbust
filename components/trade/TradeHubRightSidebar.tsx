    ...
    <div className="flex h-full min-h-0 flex-col gap-2 overflow-y-auto border-l border-[#1e2640] bg-[#0a0d14] p-[11px] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {selectedManager ? (
        <Panel title="League Intel Spotlight">
          ...
          <div className="mt-1 font-figtree text-[12px] text-[#A78BFA]">
            {displayArchetypeLabel(selectedManager.profile)}
          </div>
          <div className="mt-2">
            <div className="font-mono text-[8px] uppercase text-[#6b7a99]">Signals</div>
            <ul className="mt-0.5 space-y-0.5">
              {(selectedManager.profile.needs ?? []).slice(0, 2).map((n) => (
                <li key={n} className="font-figtree text-[11px] text-[#9aa8c4]">· Thin at {n}</li>
              ))}
              <li className="font-figtree text-[11px] text-[#9aa8c4]">
          ...
          <div className="mt-2">
            <div className="font-mono text-[8px] uppercase text-[#6b7a99]">Best Approach</div>
            <div className="font-figtree text-[11px] text-boom">
          ...
          <div className="mt-1 font-mono text-[10px] text-[#8b9bb8]">
            Trade Likelihood: <span className="text-boom">{selectedManager.tradeLikelihood}%</span>
          </div>
          <Link
            href={`/leagues/${selectedManager.leagueId}`}
            className="mt-2 inline-flex items-center gap-0.5 font-mono text-[10px] text-boom no-underline"
          >
        ...
        <Panel title="League Intel Spotlight">
          <p className="font-figtree text-[11px] text-[#6b7a99]">Building league intelligence…</p>
        </Panel>
      )}

      <Panel title="Players on the Block">
        {blockPlayers.length === 0 ? (
          <p className="font-figtree text-[11px] text-[#6b7a99]">No block signals yet.</p>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-figtree text-[12px] text-[#e8ecf4]">{p.playerName}</div>
                  ...
                  {p.verdictLabel ? (
                    <div className="mt-0.5 font-figtree text-[9px] text-[#9aa8c4]">{p.verdictLabel}</div>
                  ) : null}
                </div>
                <span
                  className="shrink-0 rounded px-1 py-px font-mono text-[8px] uppercase"
        ...
        <Link href="/players" className="mt-2 block font-mono text-[10px] text-boom no-underline">
          View All →
        </Link>
      </Panel>
      ...
      {history.length > 0 ? (
        <Panel title="Recent Trade Activity">
          {history.slice(0, 5).map((row) => (
            <div
              key={row.id}
              className="border-b border-[#1e2640]/40 py-1.5 font-figtree text-[11px] last:border-b-0"
            >
  ...
  return (
    <div className="shrink-0 overflow-hidden rounded-lg border border-[#1e2640] bg-[#0f1420]">
      <div className="border-b border-[#1e2640]/80 bg-[#0a0d14] px-3 py-1.5">
        <span className="font-figtree text-[10.5px] uppercase tracking-[1.5px] text-[#e8ecf4]">{title}</span>
      </div>
