    ...
    <aside className="row-start-2 flex flex-col overflow-hidden border-r border-border bg-surface">
      ...
      {topOffer ? (
        <div className="mx-2.5 mb-1.5 mt-2.5 shrink-0 rounded-lg border border-border bg-surface2 p-3">
          <div className="mb-0.5 font-mono text-[7.5px] uppercase tracking-[2px] text-muted">
            Quick Action
          </div>
          <div className="mb-px font-figtree text-[11px] font-bold text-boom">
            Top Offer — Action Required
          </div>
          <div className="mb-2 flex justify-between font-mono text-[8.5px] text-muted">
          ...
          <div className="mb-2.5 flex gap-2">
            {topOffer.receivePlayers.slice(0, 1).map((p) => (
              <div key={p.playerId} className="flex items-center gap-[7px]">
                ...
                <div>
                  <div className="font-figtree text-[11px] font-semibold text-text">{p.name}</div>
          ...
          <div className="mb-1 font-mono text-[7.5px] uppercase tracking-wide text-muted">
            You Give Up
          </div>
          <div className="mb-2.5 flex flex-col gap-1">
            {topOffer.givePlayers.map((p) => (
              <div key={p.playerId} className="flex items-center gap-[7px]">
                ...
                <div>
                  <div className="font-figtree text-[10.5px] font-semibold text-text">{p.name}</div>
                  <div className="font-mono text-[8px] text-muted">
                    {p.position} · {p.team}
                  </div>
                </div>
            ...
            {topOffer.givePicks.map((pick, i) => (
              <div key={`pick-${i}`} className="flex items-center gap-1.5 rounded border border-border bg-surface px-2 py-0.5 font-mono text-[8.5px]">
                <span className="rounded bg-bust/10 px-1.5 py-px text-[8px] font-bold text-bust">
                  {pick.label}
                </span>
          ...
          <div className="flex flex-col gap-[5px]">
            <button
              ...
              onClick={copyTooltip}
              className="w-full rounded-md bg-boom py-2 text-center font-figtree text-[11px] font-bold uppercase tracking-wide text-bg"
            >
              Accept
            </button>
            <button
              type="button"
              className="w-full rounded-md border border-bust bg-transparent py-2 text-center font-figtree text-[11px] font-bold uppercase tracking-wide text-bust"
            >
              Counter
            </button>
            <button
              type="button"
              className="w-full rounded-md border border-[#ef4444]/30 bg-transparent py-2 text-center font-figtree text-[11px] font-bold uppercase tracking-wide text-[#ef4444]"
            >
              Decline
            </button>
          </div>
