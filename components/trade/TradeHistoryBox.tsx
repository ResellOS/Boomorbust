  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-[7px] border border-border bg-surface">
      <div className="shrink-0 border-b border-border bg-bg px-3 py-2">
        ...
        <div className="font-mono text-[9px] text-muted">Recent completed trades</div>
      </div>
      {history.length === 0 ? (
        <div className="px-3 py-4 font-figtree text-[12px] text-muted">No trade history yet.</div>
      ) : (
        history.map((row) => (
          <div
            key={row.id}
            className="flex items-center gap-2 border-b border-border/40 px-3 py-[7px] text-[12px] last:border-b-0"
          >
            <span className="min-w-[36px] font-mono text-[9.5px] text-muted">{row.timeAgo}</span>
            <span className="flex-1 font-figtree text-[12px] text-text">
              <span className="font-semibold">{row.gaveName}</span>
              <span className="mx-1 text-muted">→</span>
              {row.receivedDisplay}
            </span>
            <span className={`whitespace-nowrap font-figtree text-[12px] font-bold ${tradeVerdictHistClass(row.verdict)}`}>
              {row.verdict}{' '}
              {row.edgeScore > 0 ? '+' : ''}
