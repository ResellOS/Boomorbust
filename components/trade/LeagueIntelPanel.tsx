  ...
  if (!manager) {
    return (
      <section className="rounded-[10px] border border-dashed border-[#1e2640] bg-[#0f1420] px-4 py-5">
        ...
        <p className="mt-2 font-figtree text-[13px] text-[#6b7a99]">
  ...
  return (
    <section className="dash-clickable-card overflow-hidden rounded-[10px] border border-[#1e2640] bg-[#0f1420] dash-boom-glow">
      <div className="flex items-center justify-between border-b border-[#1e2640]/80 px-3 py-2">
        ...
        <span className="font-mono text-[9px] text-[#6b7a99]">Manager Spotlight</span>
      </div>

      <div className="grid gap-3 p-3 sm:grid-cols-[1fr_auto]">
        <div>
          <div className="font-figtree text-[16px] font-semibold text-[#e8ecf4]">
            {manager.displayName}
            <span className="font-normal text-[#8b9bb8]"> — {manager.leagueName}</span>
          </div>
          <div className="mt-0.5 font-figtree text-[12px] text-[#A78BFA]">
            {displayArchetypeLabel(manager.profile)}
          </div>

          <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 font-mono text-[10px]">
            <div>
          ...
          <div className="mt-2">
            <div className="font-mono text-[8px] uppercase text-[#6b7a99]">Trade Tendencies</div>
            <ul className="mt-0.5 space-y-0.5">
              {tendencies.map((t) => (
                <li key={t} className="font-figtree text-[11px] text-[#9aa8c4]">· {t}</li>
              ))}
            </ul>
          </div>
          ...
          <div className="mt-2 rounded border border-[#1e2640]/50 bg-[#141929]/40 px-2 py-1.5">
            ...
            <div className="font-figtree text-[11px] leading-snug text-boom">
        ...
        <div className="flex flex-col items-end justify-between gap-2 sm:min-w-[120px]">
          <div className="text-center">
            ...
            <div className="font-mono text-[8px] uppercase text-[#6b7a99]">Deal Success Rate</div>
            <div className="font-mono text-[9px] text-[#8b9bb8]">{manager.tradeLikelihood}% trade likelihood</div>
          </div>
          <Link
            href={`/leagues/${manager.leagueId}`}
            className="dash-action-btn inline-flex items-center gap-0.5 rounded border border-boom/30 bg-boom/10 px-2.5 py-1.5 font-mono text-[10px] text-boom no-underline"
          >
