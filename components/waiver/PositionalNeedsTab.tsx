...
function NeedBar({ level }: { level: RosterGap['needLevel'] }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-1">
      ...
      <span className="text-[12px] font-semibold w-14 text-right" style={{ color, fontFamily: 'JetBrains Mono, monospace' }}>
        {level}
      </span>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="glass-card p-4">
        <h2 className="text-[14px] font-bold text-white tracking-wide mb-1">POSITIONAL NEEDS</h2>
        <p className="text-[12px] text-slate-500 mb-4">Across all your leagues — positions ranked by urgency</p>
        ...
        <div className="space-y-2">
          {loading
            ...
            : gaps.map((g) => (
              ...
              >
                <span className="text-[14px] font-bold text-white font-mono w-10" style={{ fontFamily: 'JetBrains Mono, monospace' }}>
                  {g.position}
                </span>
                <NeedBar level={g.needLevel} />
                <span className="text-[12px] text-slate-400 w-24 text-right flex-shrink-0">
      ...
      {needs.length > 0 && (
        <div className="glass-card p-4">
          <h2 className="text-[13px] font-bold text-white tracking-wide mb-3 uppercase">Priority Positions</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {needs.map((n) => {
              ...
              return (
                ...
                >
                  <div>
                    <p className="text-[15px] font-bold text-white font-mono" style={{ fontFamily: 'JetBrains Mono, monospace' }}>
                      {n.position}
                    </p>
                    <p className="text-[11px] text-slate-400">{n.count} league{n.count !== 1 ? 's' : ''}</p>
                  </div>
                  <span className="text-[12px] font-bold" style={{ color }}>{n.severity}</span>
                </div>
              );
            })}
          </div>
