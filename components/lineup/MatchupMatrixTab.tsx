...
function MatrixRow({ row, isEasy }: { row: MatchupMatrixRow; isEasy: boolean }) {
  ...
  return (
    <div className="flex items-center gap-3 py-1.5">
      <span
        className="text-[12px] font-mono w-4 text-slate-500 flex-shrink-0 text-right"
        style={{ fontFamily: 'JetBrains Mono, monospace' }}
      >
        {row.rank}
      </span>
      <span
        className="text-[13px] font-mono font-semibold w-20 flex-shrink-0"
        style={{
...
function SsasInfoBox() {
  return (
    ...
    >
      <p className="text-[13px] font-bold text-white mb-2">WHAT IS SSAS?</p>
      ...
      <button
        className="mt-3 px-4 py-1.5 rounded text-[12px] font-semibold text-white transition-opacity hover:opacity-80"
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card p-4">
        <h2 className="text-[14px] font-bold text-white tracking-wide mb-1">MATCHUP MATRIX (SSAS)</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
          {/* Easiest */}
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-[12px] font-semibold text-slate-400 uppercase tracking-wider">Easiest Matchups</p>
              <div className="flex items-center gap-3 text-[11px] text-slate-500">
                <span>SSAS Difficulty Scale</span>
              </div>
              <p className="text-[12px] font-semibold text-slate-400 uppercase tracking-wider hidden md:block">Toughest Matchups</p>
            </div>

            {loading
              ? [...Array(5)].map((_, i) => <SkeletonRow key={i} />)
              : easiest.length === 0
                ? <p className="text-[13px] text-slate-500 py-4">No matchup data available</p>
          ...
          <div className="md:hidden">
            <p className="text-[12px] font-semibold text-slate-400 uppercase tracking-wider mb-3">Toughest Matchups</p>
            {loading
              ? [...Array(5)].map((_, i) => <SkeletonRow key={i} />)
