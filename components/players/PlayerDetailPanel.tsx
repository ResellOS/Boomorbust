...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-2 font-mono text-[9px] uppercase tracking-[1.5px] text-muted">{children}</div>
  );
}
    ...
    <div className="flex min-h-full flex-col bg-[#0a0d14] pb-4">
      ...
      >
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 font-mono text-[10px] text-muted sm:grid-cols-4">
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <RecommendationFeedback
                ...
                subjectType="player"
                subjectId={pid}
                context={{ verdict: verdict.label, tfoScore: player.tfoScore }}
                className="mt-2"
              />
            </div>
            <div className="flex gap-6">
              <div className="text-center">
                <div className="font-mono text-[9px] uppercase text-muted">Confidence</div>
                <div className="font-mono text-2xl font-semibold tabular-nums text-boom">
                  <CountUpNumber value={confPct} resetKey={pid} suffix="%" />
                </div>
                <div className="font-mono text-[9px] text-muted">{confidenceLabel(player.confidenceTier)}</div>
              </div>
              <div className="text-center">
                <div className="font-mono text-[9px] uppercase text-muted">Asset Tier</div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Link
              ...
              className="rounded-md bg-boom px-4 py-2 font-mono text-[10px] uppercase text-bg no-underline"
            >
              Stage Trade
            </Link>
            <Link
              href={`/players?position=${player.position}&sort=rating`}
              className="rounded-md border border-border px-4 py-2 font-mono text-[10px] uppercase text-boom no-underline"
            >
              View Rankings
            </Link>
          </div>
      ...
      <div className="grid shrink-0 gap-3 border-b border-border p-4 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2 rounded-[10px] border border-border bg-[#0f1420] p-3 font-mono text-[10px]">
          ...
          {hasHistory ? (
            <div className="mt-2 rounded-[10px] border border-border bg-[#0f1420] p-2">
              <div className="mb-1 font-mono text-[9px] text-muted">Player Trajectory</div>
      ...
      <div className="grid shrink-0 grid-cols-2 gap-2 border-b border-border p-4 lg:grid-cols-4">
        {metricCards.map((m, i) => (
          <PanelCard key={m.headline} className="!p-2.5">
            ...
            <div className="mt-1 font-figtree text-[13px] font-semibold text-text">{m.headline}</div>
      ...
      <div className="border-b border-border p-4">
        ...
        >
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              <div className="font-mono text-[9px] uppercase text-muted">Best Leagues to Target</div>
              {tradeHints.leagues.length > 0 ? (
                <ul className="mt-1 space-y-0.5">
                  {tradeHints.leagues.map((l) => (
                    <li key={l.leagueId} className="font-figtree text-[12px] text-text">
                      · {l.leagueName}
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="mt-1 font-figtree text-[11px] text-muted">You roster {firstName} everywhere — sell window open.</p>
              )}
            </div>
            <div>
              <div className="font-mono text-[9px] uppercase text-muted">Best Managers to Approach</div>
              {tradeHints.managers.length > 0 ? (
                <ul className="mt-1 space-y-0.5">
                  {tradeHints.managers.map((m) => (
                    <li key={`${m.leagueName}-${m.name}`} className="flex justify-between font-figtree text-[12px] text-text">
                ...
                <p className="mt-1 font-figtree text-[11px] text-muted">Manager intel syncing</p>
              )}
            </div>
          </div>
          ...
          {tradeExpanded ? (
            <p className="mt-2 font-figtree text-[11px] text-muted">
          ...
          <Link
            ...
            className="mt-2 inline-flex items-center gap-0.5 font-mono text-[10px] text-boom no-underline hover:underline"
          >
      ...
      <div className="grid shrink-0 gap-3 border-b border-border p-4 lg:grid-cols-2">
        <PanelCard onClick={() => setShowAllComparables(true)}>
          <div ref={archetypeRef} id="player-archetype">
            ...
            <div className="font-figtree text-[14px] font-semibold text-boom">{archetypeLabel}</div>
          </div>
          ...
          {fullComparables.length > 0 ? (
            <div className="space-y-2">
              {fullComparables.map((cp) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate font-figtree text-[12px] text-text">{cp.fullName}</div>
                    ...
                    <div className="font-mono text-[12px] tabular-nums text-boom">{pct}%</div>
                  </button>
                );
              })}
              {!showAllComparables && comparables.length >= 3 ? (
                <button
                  ...
                  className="w-full rounded border border-border py-1.5 font-mono text-[9px] uppercase text-boom hover:bg-boom/5"
                >
            ...
            <p className="font-figtree text-[12px] text-muted">Not enough peers scored for similarity yet.</p>
          )}
        </PanelCard>
          ...
          {hasMarket ? (
            <div className="space-y-2 font-mono text-[11px]">
              ...
              <div className="mt-2 border-t border-border/50 pt-2">
                <div className="font-mono text-[9px] uppercase text-muted">Recommended Action</div>
            ...
            <p className="font-figtree text-[12px] text-muted">Market data syncing…</p>
          )}
        ...
        <PanelCard href={`/exposure`}>
          ...
          {owned ? (
            <div className="space-y-1.5 font-figtree text-[12px] text-muted">
      ...
      <div className="shrink-0 border-b border-border px-4 py-3">
        ...
        >
          {hasHistory ? (
            ...
            <p className="py-2 text-center font-figtree text-[12px] text-muted">
              ...
              <div className="space-y-2">
                {newsItems.map((n) => (
                  <div key={n.label} className="border-b border-border/40 pb-2 last:border-0">
                    <div className="font-mono text-[9px] uppercase text-muted">{n.label}</div>
                    <div className="font-figtree text-[11px] leading-snug text-text">{n.detail}</div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="font-figtree text-[12px] text-muted">No active flags — BOB monitors injuries and depth chart moves.</p>
            )}
          </div>
        </CollapsibleDossierSection>
        ...
        >
          {risks.some((r) => !r.includes('No major')) ? (
            <div className="rounded-[10px] border border-bust/20 bg-bust/5 p-2">
              {risks.map((r) => (
                <div key={r} className="mb-1 flex gap-2 font-figtree text-[11px] text-muted last:mb-0">
            ...
            <p className="font-figtree text-[11px] text-muted">No major risk flags in current profile</p>
          )}
        </CollapsibleDossierSection>
      </div>
    </div>
