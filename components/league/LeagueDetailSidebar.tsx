  ...
  return (
    <aside className="flex w-[210px] shrink-0 flex-col overflow-y-auto border-r border-border bg-surface">
      <div className="flex items-center justify-between px-3 pb-1.5 pt-2.5">
        <div className="text-[8px] font-medium uppercase tracking-[1.5px] text-muted">
          My Leagues
        </div>
      ...
      {leagues.map((league) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={`rounded-[3px] px-1.5 py-0.5 text-[8px] font-medium ${
      ...
      <Link
        href="/onboarding"
        className="mt-0.5 flex items-center gap-1.5 px-3 py-1.5 text-[11px] text-muted no-underline hover:text-text"
      >
        ＋ Add League
      </Link>
      ...
      <div className="px-3 py-2.5">
        ...
        {QUICK_ACTIONS.map((a) => (
          <Link
            key={a.label}
            href={a.href}
            className="flex items-center gap-2 py-1.5 text-[11px] text-muted no-underline hover:text-text"
          >
            <span className="w-4 text-center text-[13px]">{a.icon}</span>
            {a.label}
          </Link>
        ))}
      </div>
      ...
      <div className="m-3 rounded-md border border-boom/15 bg-gradient-to-br from-boom/[0.06] to-bust/[0.06] p-3">
        <div className="mb-2 text-[11px] font-semibold text-text">Unlock Full Power</div>
        {[
          ...
          <div key={f} className="mb-1 flex items-center gap-1.5 text-[10px] text-muted">
            <span className="text-[10px] text-boom">✓</span>
            {f}
          </div>
        ))}
        <Link
          href="/pricing"
          className="mt-2.5 block w-full rounded bg-boom py-2 text-center text-[11px] font-bold text-bg no-underline shadow-[0_0_12px_rgba(54,231,161,0.3)]"
        >
          Upgrade Now
        </Link>
      </div>
