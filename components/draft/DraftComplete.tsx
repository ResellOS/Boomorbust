  ...
  return (
    <main className="col-span-4 min-h-0 overflow-y-auto" style={{ gridColumn: '1 / -1', gridRow: 2 }}>
      <div className="mx-auto flex w-full max-w-[900px] flex-col gap-6 px-6 py-8">
        <div>
          <div className="font-figtree text-[13px] font-bold uppercase tracking-[2px] text-muted">
            Draft Complete
          </div>
        ...
        <div className="grid grid-cols-[140px_1fr] gap-4">
          ...
          >
            ...
            <div className="mt-2 text-center font-mono text-[11px] text-muted">
              Draft Grade
              <br />
        ...
        <div className="rounded-[12px] border border-border bg-surface/60 p-4">
          <div className="mb-3 font-figtree text-[12px] font-extrabold uppercase tracking-[1.5px] text-muted">
            ...
            {summary.userPicks.map((pk) => (
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="truncate font-figtree text-[13px] text-text">{pk.player.name}</span>
                </div>
                <span className="font-mono text-[12px] font-bold text-boom">
            ...
            {Array.from({ length: rounds }, (_, ri) => {
              ...
              return (
                <div key={round} className="mb-3">
                  <div className="mb-1 font-mono text-[10px] font-bold text-boom">Round {round}</div>
                  <div className="flex flex-wrap gap-2">
                    {roundPicks.map((pk) => (
                      <span
                        key={pk.overall}
                        className="rounded-[4px] border border-border/60 px-2 py-0.5 font-figtree text-[11px]"
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={onRestart}
            className="rounded-[8px] bg-boom px-6 py-2.5 font-figtree text-[14px] font-extrabold uppercase text-bg"
          >
            New Draft
          </button>
          <Link
            href="/players"
            className="rounded-[8px] border border-border px-6 py-2.5 font-figtree text-[14px] font-extrabold uppercase text-text no-underline"
          >
            Player Hub
          </Link>
        </div>
...
function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-[10px] border border-border bg-bg/40 px-3.5 py-3">
      <div className="mb-1 font-figtree text-[10px] font-bold uppercase tracking-[1.5px] text-muted">
        {label}
      </div>
      <div className="truncate font-figtree text-[16px] font-extrabold text-text">{value}</div>
