      ...
      <div className="flex-1 flex flex-col p-6 gap-4 bg-[#0A0B0E]">
        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-3">
          <div className="flex items-center gap-3">
            <span className="text-[10px] font-black italic tracking-widest text-indigo-500 uppercase">RPI PROTOCOL v2.1</span>
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981]" />
          </div>
          <div className="text-[10px] font-bold text-gray-500 uppercase tracking-tighter">Live Market Flux: 2.4ms</div>
        </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-4">
          ...
          <div className="col-span-7 bg-white/[0.02] border border-white/5 p-5 relative overflow-hidden">
            <h3 className="text-[9px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1 text-left">Portfolio Power Index</h3>
            <div className="flex items-baseline gap-3">
              <div className="text-5xl font-black tracking-tighter italic">8,422</div>
              <div className="text-emerald-400 font-bold text-[10px] tracking-widest">+840 pts</div>
            </div>
          ...
          <div className="col-span-5 bg-white/[0.02] border border-white/5 p-5 flex flex-col justify-center">
            <h3 className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-6 text-left">Core Exposures</h3>
            ...
            <div className="space-y-4">
              {ROSTERED_ASSETS.map((asset, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-5 bg-white/5 border border-white/10 flex items-center justify-center text-[9px] font-black text-gray-400">
                      {asset.team}
                    </div>
                    <div>
                      <div className="text-xs font-black uppercase italic tracking-tighter leading-none">{asset.name}</div>
                      <div className="text-[8px] font-bold text-indigo-500 uppercase mt-0.5">{asset.pos} • Unit ID: {8000 + i}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] font-black">{asset.rpi}</div>
                    <div className={`text-[8px] font-bold ${asset.trend.startsWith('+') ? 'text-emerald-500' : 'text-red-500'}`}>
                      {asset.trend}
                    </div>
                  </div>
                </motion.div>
          ...
          <div className="col-span-12 bg-black border border-white/5 p-4 flex items-center gap-6">
             <div className="shrink-0 flex flex-col items-center">
                ...
                <span className="text-[8px] font-black text-gray-500 uppercase">Rank #2</span>
             </div>
             ...
             <div className="flex-1 relative h-6 overflow-hidden">
                <AnimatePresence mode="wait">
                   ...
                   >
                      <span className="text-[10px] font-bold italic text-indigo-400 uppercase tracking-tighter">{TRADES[tradeIdx].league}</span>
                      ...
                      <span className="text-[10px] font-black text-emerald-500">{TRADES[tradeIdx].powerShift}</span>
                   </motion.div>
                </AnimatePresence>
             </div>
          </div>
