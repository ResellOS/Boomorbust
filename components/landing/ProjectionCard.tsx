    ...
    >
      <div className="flex items-start justify-between gap-2 border-b border-white/[0.06] pb-3">
        <div className="min-w-0">
          <p className="truncate text-[16px] font-bold text-white">{p.playerName}</p>
          <p className="mt-0.5 text-[12px] text-[#64748B]">
            {p.position} · {p.team} · WK <span className="font-mono tabular-nums">{p.week}</span>
          </p>
        </div>
        <div className="shrink-0 text-right">
          <p className="text-[11px] font-bold uppercase tracking-wide text-[#64748B]">Rating</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-[12px]">
        <div className="rounded-lg border border-white/[0.06] bg-white/[0.03] px-2.5 py-2">
          <p className="text-[10px] uppercase tracking-wide text-[#64748B]">Projection</p>
        ...
        <div className="rounded-lg border border-white/[0.06] bg-white/[0.03] px-2.5 py-2">
          <p className="text-[10px] uppercase tracking-wide text-[#64748B]">{p.matchupLabel}</p>
          <p className="mt-0.5 font-bold text-white">
      ...
      <div className="mt-2 flex flex-wrap gap-1">
        {p.flags.map((f) => (
          <span
            key={f}
            className="rounded border border-white/[0.08] bg-white/[0.05] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[#94A3B8]"
          >
            {f.replace(/_/g, ' ')}
          </span>
        ))}
      ...
      <p className="mt-3 border-t border-white/[0.06] pt-3 text-[13px] leading-snug text-[#94A3B8]">{p.reasoning}</p>
    </div>
  );
}
